...
use super::{motion::Move, screen::Screen, vector::Vector2D};
...
impl<'a> Point2D<'a> {
    ...
    fn add_vector(&self, vec: Vector2D) -> Result<Self, &str>
    where
        Self: Sized,
    {
...
impl<'a> Point2DLike<'a> for Point2D<'a> {
    ...
    fn new(context: &'a Screen, x: f64, y: f64) -> Option<Point2D<'a>> {
